var str =
  "JS코드는 함수가 아닌 JS 파일 또는 script태그 내부에 바로 작성시" +
  "HTML 랜더링 시 바로 수행된다.";
//전역상태는 함수안에 넣어서 코드를 불러오지 않아도
//페이징 랜더링이 가능하다.
console.log(str);
var num1 = 100;
num2 = 200;
...
function test() {
  ...
  if (true) {
    var num5 = 500; //지역변수
    //js에서 if안에 지역변수는 if밖 function안에서 사용 가능
    num6 = 600; //전역변수
  }
  console.log("if문 종료 후 num5:" + num5); //function지역변수
...
//스크립트 언어는 중간에 에러 발생 시 해석을 멈춤
//따라서 밑에 코드는 콘솔에 뜨지 않음
//console.log("함수 밖 num4:" + num4);
...
function typeTest() {
  ...
  //const와 let은 블록범위라 if밖이나 블록밖에서 찍히지 않음
  //   console.log(temp1);
  //   console.log(temp2);
  //   console.log(temp3);
  ...
  typeBox.innerHTML += "";
  let name; //undifined
  //해당 변수의 자료형을 반환하는 연산자: typeof 변수명;
  ...
  for (let i = 0; i < arr.length; i++) {
    typeBox.innerHTML +=
      "<br><br>arr[" + i + "] :" + arr[i] + "/" + typeof arr[i];
  }
  ...
  console.log(user);
  typeBox.innerHTML += "<br><br>user.age : " + user.age + "/" + typeof user.age;
  ...
  //함수호출이 아닌 함수명만 불러서 타입이 안나옴
  //->함수에 작성된 코드 자체가 반환
  typeBox.innerHTML += "<br><br>fnSum : " + fnSum + "/" + typeof fnSum;

  //함수명(매개변수) == 함수호출(함수 코드 수행됨)
  typeBox.innerHTML +=
    "<br><br>fnSum(1,2) : " + fnSum(1, 2) + "/" + typeof fnSum();
