...
document.getElementById("check1").addEventListener("click", function () {
  ...
  //"script"와 일치하는 문자열이 있는지 검사하는 정규식 객체를 생성

  const regEx2 = /java/;
  //"Java"와 일치하는 문자열이 있는지 검사하는 정규식 객체를 생성

  const str1 = "저희는 지금 javascript를 공부하고 있습니다.";

  const str2 = "java, db, html, scc, js 모두 사용중 입니다.";
  ...
  console.log("regEx1.test(str2): " + regEx2.test(str2));
  console.log("regEx1.test(str2): " + regEx2.exec(str2));

  console.log("regEx1.test(str3): " + regEx2.test(str3));
  console.log("regEx1.test(str3): " + regEx2.exec(str3));
...
//이름 유효성검사
document.getElementById("nametest").addEventListener("keyup", function () {
  let nametest = document.getElementById("result1");

  //한글 2글자 이상6글자 이하
  //자음/모음 경우 제외
  const regEx1 = /^[가-힣]{2,6}$/;
  if (regEx1.test(this.value)) {
    nametest.innerText = "유효한 이름 형식입니다.";
    // nametest.style.color = "green";
    ...
    nametest.classList.add("confirm");
    nametest.classList.remove("error");
  } else {
    nametest.innerText = "잘못된 형식입니다.";
    // nametest.style.color = "red";
    nametest.classList.add("error");
    nametest.classList.remove("confirm");
  }
});
