...
document.getElementById("btn2").addEventListener("click", function () {
  ...
  const stdlist = []; //비어있는배열 생성

  //2. 생성자 함수 호출 : new 생성자함수명(매개변수)
  stdlist.push(new Student(1, 2, "홍길동"));
  stdlist.push(new Student(1, 3, "김민재"));
  stdlist.push(new Student(1, 4, "손흥민"));
  stdlist.push(new Student(1, 5, "박지성"));
...
function Student(grade, ban, name) {
  // JS 함수의 매개변수는 let, const var같은 키워드 적지 않음
...
document.getElementById("btn3").addEventListener("click", function () {
  ...
  console.log(std); //객체로 뜸
  console.log(JSON.stringify(std)); //객체를 JSON으로 변환
  ...
  console.log(JSON.parse(member)); //객체로 변환
});
